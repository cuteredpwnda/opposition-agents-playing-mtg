...
\begin{tikzpicture}[
    ...
    box/.style ={draw, rounded corners=2pt, align=center, minimum height=0.7cm,
                 inner sep=4pt},
    data/.style={box, fill=gray!8,   minimum width=2.5cm},
    sym/.style ={box, fill=blue!8,   minimum width=3.4cm},
    learn/.style={box, fill=orange!12, minimum width=3.4cm},
    agent/.style={box, fill=green!10, minimum width=3.0cm, font=\scriptsize},
    grp/.style ={draw, dashed, rounded corners=4pt, inner sep=8pt},
    flow/.style={->, thick, blue!50!black},
    backflow/.style={->, thick, dashed, red!60!black},
    label/.style={font=\scriptsize\itshape, midway, fill=white, inner sep=1pt}
  ]

  % --- Column 1: external data sources --------------------------------
  \node[data] (scry) at (0,0) {Scryfall\\(card DB)};
  \node[data, below=4pt of scry] (rules) {Comprehensive\\Rules};
  \node[data, below=4pt of rules] (sb)    {Commander\\Spellbook};
  \node[data, below=4pt of sb]    (decks) {Decklists\\(\texttt{data/decks})};

  % --- Column 2: symbolic substrate ----------------------------------
  \node[sym, right=2.0cm of scry]  (kg)    {\textbf{Knowledge Graph}\\
        Neo4j $\!+\!$ n10s (OWL2)\\$+\!$ APOC $+$ GraphSAGE};
  \node[sym, below=12pt of kg]     (eng)   {\textbf{Game Engine}\\
        zones / stack / priority\\phases / combat / triggers /\\state-based actions};
  \node[sym, below=12pt of eng]    (rb)    {\textbf{Replay Buffer}\\trajectories $(s_t, a_t, r_t)$};

  % --- Column 3: learned substrate -----------------------------------
  \node[learn, right=1.8cm of kg]  (V)  {V: state encoder\\(Set-Transformer + KG ctx)};
  \node[learn, below=8pt of V]     (M)  {M: MDN-LSTM dynamics\\$\!+\!$ JEPA target head};
  \node[learn, below=8pt of M]     (C)  {C: controller\\(policy + value heads)};
  \node[grp, fit=(V)(M)(C), label={[font=\small\bfseries]above:World Model}] (wm) {};

  % --- Column 4: agents -----------------------------------------------
  \node[agent, right=1.6cm of V]   (a1) {Random / Heuristic};
  \node[agent, below=4pt of a1]    (a2) {KG-Heuristic};
  \node[agent, below=4pt of a2]    (a3) {Ollama LLM};
  \node[agent, below=4pt of a3]    (a4) {World-Model};
  \node[agent, below=4pt of a4]    (a5) {Active Inference};
  \node[agent, below=4pt of a5]    (a6) {LLM-Fusion};
  \node[grp, fit=(a1)(a6), label={[font=\small\bfseries]above:Agent Zoo}] (agents) {};

  % --- arrows: data into symbolic substrate ---------------------------
  \draw[flow] (scry.east)  -- ++(0.6,0) |- (kg.west)  node[label,xshift=-0.3cm]{cards};
  \draw[flow] (rules.east) -- ++(0.6,0) |- (eng.west) node[label,xshift=-0.3cm]{rules};
  \draw[flow] (sb.east)    -- ++(0.6,0) |- (kg.west)  node[label,xshift=-0.3cm]{combos};
  \draw[flow] (decks.east) -- ++(0.6,0) |- (eng.west) node[label,xshift=-0.3cm]{game cfg};

  % --- engine <-> KG (lookup) ----------------------------------------
  \draw[<->, thick, gray] (kg.south) -- (eng.north) node[label]{card lookup};

  % --- engine -> replay buffer (trajectories) ------------------------
  \draw[flow] (eng.south) -- (rb.north) node[label]{$(s_t,a_t,r_t)$};

  % --- WM training: replay -> V/M/C ----------------------------------
  \draw[flow] (rb.east) -- ++(0.6,0) |- (M.west) node[label,xshift=-0.3cm]{train};
  \draw[flow] (kg.east) -- ++(0.5,0) |- (V.west) node[label,xshift=-0.3cm]{KG ctx};

  % --- agent decide_action loop -------------------------------------
  \draw[flow] (eng.east)   to[bend left=18] node[label]{state, legal\_actions} (a3.west);
  \draw[backflow] (a3.west) to[bend left=18] node[label]{action} (eng.east);

  % --- agents query WM -----------------------------------------------
  \draw[flow, dotted] (a4.west) -- ++(-0.3,0) |- (C.east);
  \draw[flow, dotted] (a6.west) -- ++(-0.3,0) |- (C.east);
  \draw[flow, dotted] (a2.west) -- ++(-0.3,0) |- (kg.east);

\end{tikzpicture}
\caption{Data-flow architecture. \textbf{Grey} boxes are external data sources; \textbf{blue} the symbolic substrate (engine, knowledge graph, replay buffer); \textbf{orange} the learned world model (V+M+C with a JEPA target head and a knowledge-graph context channel); \textbf{green} the agent zoo. Solid arrows are forward data flow; the red dashed arrow is the agent's chosen action returned to the engine; dotted arrows mark the lookups an agent performs at decision time. The engine is the only component allowed to mutate game state; agents read but do not write.}
\label{fig:stack}
\end{figure}
...
\begin{enumerate}[leftmargin=1.2em, itemsep=2pt]
  ...
  \item An \emph{active-inference LLM-fusion agent} that chooses actions by minimising expected free energy (EFE) combining pragmatic value, epistemic value, and a calibrated LLM prior; a self-play training loop with Elo rating, opponent-pool sampling and champion-vs-challenger promotion is provided.
\end{enumerate}
