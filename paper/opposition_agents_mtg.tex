...
\paragraph{Checkpoint-benchmark status.} We have \emph{not yet} run a clean benchmark in this workspace showing archived, fully trained world-model checkpoints outperforming \texttt{heuristic}, \texttt{random}, or pure \texttt{llm} baselines. The current results establish that the post-fix world-model agent can play normally and tie the heuristic baseline on the tested Modern matchup, but the repository did not contain persisted training checkpoints at the time of writing. To close that gap we added a dedicated checkpoint benchmark path (\texttt{scripts/benchmark\_trained\_agents.py}) and an active-inference-inspired checkpoint selector, so the next training run can be evaluated as a true trained-vs-baseline comparison rather than a smoke test.
...
\begin{enumerate}[leftmargin=1.2em, itemsep=1pt]
  ...
  \item \textbf{Trained-agent benchmark.} Run the newly added checkpoint benchmark workflow on persisted world-model checkpoints so that we can answer the central empirical question directly: do trained world-model agents beat \texttt{random}, \texttt{heuristic}, pure \texttt{llm}, and \texttt{active\_inference} baselines on held-out matchups? The missing ingredient is no longer harnessing but archived checkpoints.
  \item \textbf{Active-inference checkpoint selection.} Extend the current active-inference-inspired selector from post-hoc checkpoint ranking to an in-training promotion rule, where world-model candidates are selected not just by win rate but by an expected-free-energy objective combining performance, cross-baseline robustness, and decision cost.
