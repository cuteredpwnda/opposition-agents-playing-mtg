...
% Compile: pdflatex kg_jepa_world_model.tex (twice for ToC)
...
% --- Slide 3 -------------------------------------------------------
\begin{frame}[fragile]{This Repository: opposition-agents-playing-mtg}
  A complete research framework --- not a toy prototype.
  \vspace{0.3cm}
  \begin{center}
  \begin{tikzpicture}[
      every node/.style={font=\footnotesize},
      pkg/.style={draw, rounded corners, fill=#1, minimum width=3.2cm,
                  minimum height=0.7cm, align=center},
    ]
    \node[pkg=mtgred!12]     (engine) at (0, 0)    {\textbf{src/engine/}\\Game Rules Engine};
    \node[pkg=mtgblue!12]    (agents) at (4.5, 0)  {\textbf{src/agents/}\\Agent Zoo};
    \node[pkg=mtggreen!12]   (wm)     at (9, 0)    {\textbf{src/world\_model/}\\V+M+C World Model};
    \node[pkg=kgpurple!12]   (kg)     at (0, -2)   {\textbf{src/knowledge/}\\Neo4j KG + Ontology};
    \node[pkg=jepaorange!12] (train)  at (4.5, -2) {\textbf{src/training/}\\Self-Play + Dream};
    \node[pkg=mtggold!15]    (data)   at (9, -2)   {\textbf{data/ontology/}\\OWL v1.1 + Scryfall};
    ...
    \draw[-{Stealth}, thick, dashed] (kg) -- (wm)
      node[midway, above, sloped, font=\scriptsize] {KG fusion};
  \end{tikzpicture}
  \end{center}
  \vspace{0.2cm}
  \begin{columns}[T]
...
\begin{frame}{The Agent Zoo: Multiple Minds, One Game}
  ...
  \begin{tikzpicture}[
      ...
      agent/.style={draw, rounded corners, minimum width=2.4cm,
                    minimum height=1.6cm, align=center},
      >=Stealth,
    ]
    \node[agent, fill=lgray]         (rand)   at (0,0)       {\textbf{RandomAgent}\\Fast baseline\\No learning};
    ...
    \node[draw, dashed, rounded corners,
          fit=(rand)(llm)(wma)(active)(fusion)(combo),
          inner sep=8pt,
          label={[font=\footnotesize]below:Uniform interface:
                 \texttt{agent.choose\_action(state, actions)}}] {};
  ...
  \vspace{0.2cm}
  \begin{center}
    Any agent can play against any other
    $\Rightarrow$ tournaments, ELO ratings, architecture comparison.
  \end{center}
\end{frame}
...
\begin{frame}{The World Model: Imagination as Strategy}
  ...
  \begin{tikzpicture}[
    ...
    \node[block, fill=mtgblue!15]  (V) at (0,0)
      {\textbf{V: Encoder}\\features $\to \mu, \sigma$\\$z \in \mathbb{R}^{256}$\\VAE + Set Transf.};
    \node[block, fill=mtggreen!15] (M) at (4,0)
      {\textbf{M: Dynamics}\\$P(z_{t+1} \mid z_t, a_t, h_t)$\\MDN-LSTM\\Gaussian mixture};
    \node[block, fill=mtgred!15]   (C) at (8,0)
      {\textbf{C: Controller}\\$a = f(z, h)$\\$\sim$1K params\\CMA-ES / PG};

    \node[above=0.8cm of V, font=\footnotesize] (input)
      {GameState $\to$ GameTokenizer};
    \draw[->, thick] (input) -- (V);

    \draw[->, thick] (V) -- (M) node[midway, above] {$z_t$};
    \draw[->, thick] (M) -- (C) node[midway, above] {$z_{t+1}, h_t$};

    \node[block, fill=mtggold!20, minimum width=6cm,
          below=1.2cm of M] (dream)
      {\textbf{Dream Environment}\\Roll out $N$ imaginary games in latent space\\
       $\sim$1000$\times$ faster than real engine\\
       Train C entirely inside dreams};
...
% --- Slide 7 -------------------------------------------------------
\begin{frame}[fragile]{Training: Self-Play $\to$ Dream $\to$ Deploy}
  \begin{center}
  \begin{tikzpicture}[
      every node/.style={font=\scriptsize},
      stage/.style={draw, rounded corners, fill=#1, minimum width=4.5cm,
                    minimum height=1.1cm, align=center, thick},
      >=Stealth,
    ]
    \node[stage=lgray] (s1) at (0, 2.4)
    ...
    \node[draw, dashed, rounded corners, fill=mtgred!8,
          text width=4cm, align=center,
          right=0.8cm of s3, font=\scriptsize]
      (limit) {\textbf{But:} the model only knows\\what it has seen.\\
               28,000 cards $\times$ interactions\\$=$ billions of unknowns.};
...
\begin{frame}[fragile]{The Limitation: Knows Only What It Has Seen}
    ...
    \begin{column}{0.55\textwidth}
      ...
      \begin{tikzpicture}[every node/.style={font=\scriptsize}]
        ...
        \node[font=\scriptsize, anchor=north west]
          at (0,3.5) {Total interaction space ($\sim$billions)};
        \node[font=\tiny, anchor=west, mtggreen!80!black]
          at (1.1, 0.4) {Training coverage ($<0.001\%$)};

        \foreach \x/\y in {2.5/2.8, 4.0/1.5, 1.5/2.0, 3.5/3.0,
                           4.5/0.5, 0.8/2.8, 3.0/0.8} {
  ...
  \begin{center}
    \begin{tikzpicture}
      \node[draw, rounded corners, fill=mtgred!10,
            text width=10cm, align=center, font=\small] {
        The world model is powerful but \textbf{blind}
...
\begin{frame}{What World Models Do Well}
  \begin{columns}[c]
    \begin{column}{0.4\textwidth}
      ...
      \begin{tikzpicture}[
          every node/.style={font=\scriptsize},
          sbox/.style={draw, rounded corners, fill=mtgblue!10,
                       minimum width=3.8cm, align=center},
          >=Stealth,
        ]
        \node[sbox] (enc)   at (0, 0)    {Encode to latent $z$ \quad (V)};
        \node[sbox] (pred)  at (0, -0.9) {Predict next $z$ \quad (M)};
        \node[sbox] (act)   at (0, -1.8) {Choose best action \quad (C)};
        \node[sbox] (dream) at (0, -2.7) {Dream 100 futures \quad (Dream)};
...
\begin{frame}{What a Knowledge Graph Knows}
  ...
  \begin{tikzpicture}[
      every node/.style={font=\scriptsize},
      branch/.style={draw=none, text=kgpurple!80!black,
                     font=\scriptsize\bfseries},
      leaf/.style={draw=none, font=\scriptsize},
    ]
    \node[branch] (root) at (0,0) {MTG Knowledge Graph (Neo4j)};
    \node[leaf, anchor=west] at (0.5, -0.6)
      {28,000+ cards with full OWL ontology (types, mana, oracle text)};
    \node[leaf, anchor=west] at (0.5, -1.2)
      {700+ known infinite combos (Commander Spellbook)};
    \node[leaf, anchor=west] at (0.5, -1.8)
      {Synergy scoring via graph algorithms (APOC)};
    \node[leaf, anchor=west] at (0.5, -2.4)
      {Archetype graphs (aggro / control / combo / midrange)};
    \node[leaf, anchor=west] at (0.5, -3.0)
      {Format legality, meta trends, judge rulings};
  \end{tikzpicture}
  \end{center}
  \vspace{0.4cm}
  \begin{center}
    This knowledge is \textbf{complete, correct, and permanent}.\\[4pt]
    It doesn't need training games to learn that Thassa's Oracle\\
    wins on an empty library. It already knows.
  \end{center}
\end{frame}
...
\begin{frame}{The Complementarity: System 1 + System 2}
  ...
  \begin{tikzpicture}[
      every node/.style={font=\footnotesize},
      sysbox/.style={draw, thick, rounded corners,
    ...
    \draw[{Stealth}-{Stealth}, very thick, mtggold]
      (s1.east) -- (s2.west)
      node[midway, below=0.3cm, font=\small\bfseries, mtggold!80!black]
...
\begin{frame}{Benefit 1: Semantic Grounding}
  \textbf{Problem:} $z \in \mathbb{R}^{256}$ means nothing.
  Dimension 47 might be ``mana advantage'' --- or not.
  \vspace{0.3cm}
  ...
  \begin{tikzpicture}[
      every node/.style={font=\scriptsize},
      sbox/.style={draw, rounded corners, fill=kgpurple!10,
                   minimum height=0.6cm, align=center},
      >=Stealth,
    ]
    \node[sbox] (kg)   at (0,0)   {KG Node: Lightning Bolt};
    \node[sbox] (feat) at (4.5,0) {\textsc{instant}, R, 1 CMC, 3-damage};
    \node[sbox] (gnn)  at (9,0)   {GNN $\to \mathbb{R}^{128}$};
  ...
  \end{itemize}
  \vspace{0.2cm}
  \begin{center}Training convergence improves dramatically.\end{center}
\end{frame}
...
\begin{frame}{Benefit 2: Zero-Shot Reasoning About New Cards}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Without KG:}\\
      Agent misplays new cards for weeks until enough training games
      are collected.
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{With KG:}\\
  ...
  \begin{center}
    ...
    If two new cards form a loop, the KG detects it via
    graph traversal \textit{before the agent ever plays them}.
  \end{center}
\end{frame}
...
\begin{frame}{Benefit 3: Opponent Modeling Beyond Statistics}
  ...
  \begin{tikzpicture}[
      every node/.style={font=\scriptsize},
      obs/.style={draw, rounded corners, fill=lgray,
                  minimum width=3.5cm, align=left},
      inf/.style={draw, rounded corners, fill=kgpurple!10,
                  minimum width=5cm, align=left},
      >=Stealth,
    ]
    \node[obs] (o1) at (0, 0)
      {Hallowed Fountain\\(untapped) + Island};
    \node[inf] (i1) at (7, 0)
      {KG: Azorius Control archetype\\$\Rightarrow$ counterspell mana open};
    \draw[->, thick] (o1) -- (i1)
      node[midway, above, font=\scriptsize\itshape] {KG query};

    \node[obs] (o2) at (0, -1.5)
      {Turn 1: Thoughtseize};
    \node[inf] (i2) at (7, -1.5)
      {KG: Black midrange/combo\\$\Rightarrow$ expect hand disruption};
    \draw[->, thick] (o2) -- (i2)
      node[midway, above, font=\scriptsize\itshape] {KG query};
  \end{tikzpicture}
  \end{center}
  ...
  \begin{center}
    ...
    it says \textbf{why} and \textbf{what comes next}.
  \end{center}
\end{frame}
...
\begin{frame}{Benefit 4: Sample Efficiency $\times$ 10}
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      ...
      \end{center}
      \vspace{0.3cm}
      \footnotesize $C$ (clusters) $\ll$ $N$ (interactions).\\
...
\begin{frame}{Benefit 5: Closing the Explainability Gap}
  \textbf{Problem:} Dream search evaluated 500 futures.
  Result: ``action 7 had highest value.''
  \vspace{0.3cm}

  \textbf{KG-augmented explanation:}
  \begin{enumerate}\small
    \item Dream search found: \textit{Play Goblin Charbelcher, activate for 12 (lethal)}
    \item KG lookup: No lands in deck $\Rightarrow$ Charbelcher deals
          cards-until-land damage
    \item KG: This is the ``Belcher'' combo --- known goldfish deck
    \item Explanation: \textit{``Agent held Force of Will,
          countering Charbelcher activation.''}
  \end{enumerate}
  \vspace{0.3cm}

  \begin{center}
    Matters for \textbf{trust}, \textbf{debugging}, and \textbf{education}.
  \end{center}
\end{frame}

% --- Slide 19 ------------------------------------------------------
\begin{frame}[fragile]{Benefit 6: Format Transfer Without Retraining}
  ...
  \begin{tikzpicture}[
      every node/.style={font=\scriptsize},
      cbox/.style={draw, rounded corners, fill=#1, text width=4.5cm, align=left},
      >=Stealth,
    ]
    \node[cbox=kgpurple!10] (c) at (0,0)
      {\textbf{Format-independent (KG):}\\
       Card type system\\Core mechanics (damage, mana, ETB)\\
       Combo archetypes\\Strategic primitives};
    \node[cbox=mtgblue!10] (v) at (7, 0)
      {\textbf{Format-specific (WM):}\\
       Frequency distributions\\Meta weighting\\
       Card pool overlap\\Speed of format};
...
\begin{frame}{The Three Memory Systems Together}
  ...
  \begin{tikzpicture}[
      every node/.style={font=\scriptsize},
      mem/.style={draw, thick, rounded corners, minimum width=2.5cm,
                  minimum height=2cm, align=center},
      >=Stealth,
    ]
    \node[mem, fill=mtgblue!12] (wm) at (0,0)
      {\textbf{World Model}\\Working Memory\\[3pt]``What WILL\\happen?''\\[3pt]\tiny Seconds / Latent};
    \node[mem, fill=kgpurple!12] (kg) at (4,0)
      {\textbf{Knowledge}\\~\textbf{Graph}\\Semantic Memory\\[3pt]``What IS true?''\\[3pt]\tiny Permanent / Cypher};
    \node[mem, fill=mtggreen!12] (llm) at (8,0)
      {\textbf{LLM}\\Linguistic\\Reasoning\\[3pt]``What SHOULD\\I do?''\\[3pt]\tiny Per-query};

    \node[draw, thick, fill=mtggold!20, rounded corners,
          minimum width=1.5cm] (dec) at (4, -2.5)
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    Knowledge Graph & Neocortex         & Novel situations, combos \\
...
\begin{frame}{What Is LeWM?}
  ...
  \begin{tikzpicture}[
      ...
      pred/.style={draw, thick, rounded corners, fill=jepaorange!15,
                   minimum width=2cm, minimum height=0.7cm, align=center},
      >=Stealth,
    ]
    % Classic
    \node[font=\footnotesize\bfseries] at (-1.2, 2) {Classic (DIAMOND):};
    ...
    % JEPA
    \node[font=\footnotesize\bfseries] at (-1.2, 0) {JEPA (LeWM):};
    ...
    \node[draw, dashed, rounded corners, fill=jepaorange!8,
          text width=5cm, align=center] at (7.5, -0.3)
      {Loss: $\|\hat{z}_{t+1} - z_{t+1}\|^2
       + \beta \cdot \mathrm{KL}(q(z) \| \mathcal{N}(0,I))$};
  \end{tikzpicture}
  \end{center}
  \vspace{0.2cm}
  \begin{itemize}
    \item No decoder needed --- predict in latent space directly
    \item Gaussian regularizer prevents collapse
          (no EMA, no stop-grad on targets needed)
...
\begin{frame}{LeWM Numbers That Matter}
  ...
  \vspace{0.4cm}
  \begin{center}
    \textbf{Key insight:} LeWM's latent space provably encodes
    physical/semantic structure (confirmed by probing).\\
...
\begin{frame}{The Surprise Detection Bonus}
  ...
  \begin{tikzpicture}[
      every node/.style={font=\scriptsize},
      scenario/.style={draw, rounded corners, fill=jepaorange!10,
                       text width=5.2cm, align=left},
      response/.style={draw, rounded corners, fill=kgpurple!10,
                       text width=5.2cm, align=left},
      >=Stealth,
    ]
    \node[scenario] (s1) at (0, 0)
      {\textbf{Scenario:} 4/4 creature survives Lightning Bolt\\
       $\Rightarrow$ High surprise score};
    \node[response] (r1) at (7.8, 0)
      {\textbf{KG:} Creature has ``Indestructible''\\
       $\Rightarrow$ Update dynamics model};

    \node[scenario] (s2) at (0, -1.8)
      {\textbf{Scenario:} Opponent gains 20 life in one turn\\
       $\Rightarrow$ High surprise score};
    \node[response] (r2) at (7.8, -1.8)
      {\textbf{KG:} Aetherflux Reservoir + storm\\
...
\begin{frame}{Dual-Input JEPA Architecture}
  ...
  \begin{tikzpicture}[
    ...
    % Tokenizer
    \node[block, fill=lgray, minimum width=2.5cm] (tok) at (1.5, 3.5)
      {GameTokenizer};
    \draw[->, thick] (gs) -- (tok);

    % KG Encoder
    \node[block, fill=kgpurple!15, minimum width=2.5cm] (kge) at (1.5, 2.0)
      {KGContextEncoder\\{\tiny GNN + Self-Attention}};
    \draw[->, thick] (vc) -- (kge);

    % StateEncoder
    \node[block, fill=mtgblue!15, minimum width=3.5cm,
          minimum height=1.8cm] (enc) at (6, 2.75)
    ...
    % JEPA predictor
    \node[block, fill=jepaorange!15, minimum width=2.5cm,
          minimum height=1.2cm] (jepa) at (12, 3.8)
      {\textbf{JEPA Predictor}\\{\tiny Transformer}\\
       $\hat{z}_{t+1} = f(z_t, a_t)$\\{\tiny Loss: MSE + $\beta$KL}};

    % MDN-LSTM
    \node[block, fill=mtggreen!15, minimum width=2.5cm,
          minimum height=1.2cm] (mdn) at (12, 1.5)
      {\textbf{MDN-LSTM (M)}\\
       $P(z_{t+1} \mid z_t, a_t, h_t)$\\{\tiny Loss: NLL}};
    ...
    \node[draw, dashed, rounded corners, fill=mtggold!10,
          text width=3.8cm, font=\tiny, align=center]
      at (6, 0.4)
      {KG fuses via \textbf{residual addition}\\
       Game state dominates;\\
       KG provides additive correction\\
       Backward compatible ($\text{kg}=\text{None} \to$ vanilla)};
  \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]{LeWM + KG: The Killer Combination}
  ...
  \begin{tikzpicture}[
      ...
      sbox/.style={draw, thick, rounded corners, fill=#1,
                   minimum width=8cm, minimum height=0.65cm, align=left},
      >=Stealth,
    ]
    \node[sbox=kgpurple!10] (s1) at (0, 3)
      {\textbf{1.} Build KG node embeddings
       (GNN over card + synergy graph) $\to \mathbb{R}^{128}$};
    \node[sbox=mtgblue!10] (s2) at (0, 2)
      {\textbf{2.} Initialize encoder with KG embeddings
       $\to$ structured space on day 1};
    \node[sbox=jepaorange!10] (s3) at (0, 1)
      {\textbf{3.} Train with prediction + KL losses
       $\to$ refine structure from trajectories};
    \node[sbox=mtggreen!10] (s4) at (0, 0)
      {\textbf{4.} Probe latent space
       $\to$ mana advantage, board presence, combo proximity};
    \node[sbox=mtgred!10] (s5) at (0, -1)
      {\textbf{5.} Surprise detection flags KG gaps
       $\to$ self-correcting knowledge loop};
...
\begin{frame}{Implementation Status}
  ...
  \begin{tikzpicture}[
      every node/.style={font=\scriptsize},
      phase/.style={draw, thick, rounded corners, text width=12cm,
                    align=left, inner sep=6pt},
    ]
    \node[phase, fill=mtggreen!10] (p1) at (0, 2.8)
      {\textbf{Phase 1: Dual-Input JEPA Architecture}
       \hfill \textcolor{mtggreen}{\checkmark\ DONE}\\
    ...
    \node[phase, fill=jepaorange!10] (p4) at (0, -1.6)
      {\textbf{Phase 4: Validation \& Deployment}
       \hfill \textcolor{jepaorange}{IN PROGRESS}\\
...
\begin{frame}{Final Thesis}
  \begin{center}
    \large\bfseries World Model + Knowledge Graph + JEPA\\
    = a genuinely novel agent architecture
  \end{center}
  \vspace{0.3cm}
  \begin{center}
  \begin{tikzpicture}[
      every node/.style={font=\small},
      row/.style={draw, thick, rounded corners, text width=11cm,
                  align=left, minimum height=0.9cm, inner sep=5pt},
    ]
    \node[row, fill=lgray] at (0, 2.4)
      {\textbf{Traditional RL:} learns from scratch $\to$ millions of games
       $\to$ brittle to new cards};
    \node[row, fill=lgray] at (0, 1.2)
      {\textbf{LLM agent:} knows symbolically $\to$ can't plan ahead
       $\to$ slow $\to$ expensive};
    \node[row, fill=mtgblue!10] at (0, 0)
      {\textbf{World Model (today):} dreams fast $\to$ learns from play
       $\to$ limited by training dist.};
    \node[row, fill=mtggreen!15] at (0, -1.4)
      {\textbf{WM + KG + JEPA (ours):} dreams 48$\times$ faster
       \textbullet\ KG-structured latent space\\
       \hspace{2.8cm} zero-shot via KG \textbullet\ self-correcting
       \textbullet\ explainable \textbullet\ format-portable};
  \end{tikzpicture}
  \end{center}
  \vspace{0.4cm}
  \begin{center}
...
\begin{frame}{References}
  ...
  \begin{itemize}
    ...
    \item Alonso, Jelley et al.\ (2024). ``DIAMOND: Diffusion for World
          Modeling.'' \textit{NeurIPS 2024 Spotlight}
    \item MTG Knowledge Graph:
          \texttt{data/ontology/mtg-ontology-v1.1.owl}
  \end{itemize}
\end{frame}
